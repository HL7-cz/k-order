...
def diagram(header, composition, request, bundle):
    lines = ['@startuml', "' Generated by scripts/sync-header-mapping.py",
             'hide circle', 'hide stereotype', 'hide members', 'hide methods',
             'skinparam linetype ortho',
             'skinparam class {',
             'BackgroundColor<<LogicalModel>> #APPLICATION',
             'BackgroundColor<<Profile>> #TECHNOLOGY', '}',
             'legend right', '| <#APPLICATION>  Logical Model |',
    ...
    for index, (name, link) in enumerate(profiles):
        ...
        if index:
            lines.append(f'Target{index - 1} -d[hidden]-> Target{index}')
    lines.append('@enduml')
    return '\n'.join(lines) + '\n'
...
def main():
    ...
    parser.add_argument('--order-only', action='store_true', help='Only update/check this Order checkout')
    args = parser.parse_args()
    img = args.img_root.resolve()
    if not args.order_only and not (img / 'sushi-config.yaml').is_file():
    ...
    print(f'{len(rows)} shared rows; {page_count} header pages checked' if args.check else f'Updated {len(changed)} files from {len(rows)} shared rows')
