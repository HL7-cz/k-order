...
K and FT reference this profile from `Composition.section[clinicalIndication].entry`. Logical model A.2.2.2 maps to `Condition.code`; A.2.2.3 maps to the section narrative and, when a Condition represents the clinical problem, its `code.text`. The section continues to use the general clinical-indication code LOINC 104720-8.
