...
* section[supportingInformation].entry ^definition = "References to the structured clinical resources summarized by this section. Reuse resource instances already referenced elsewhere in the document. The open list also permits the declared general Observation profile even when no dedicated slice is defined. CarePlan resources belong in section[carePlan]."
